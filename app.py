...
st.sidebar.markdown("---")
if st.sidebar.checkbox("🔓 Modo Admin (Deletar Forçado)"):
                    ...
                    with col3:
                        ...
                        if fc > 0:
                            ...
                            if st.session_state.get('admin_mode', False):
                                if st.button("🔥 FORÇAR", key=f"fdel_c{c.id}"):
                                    fichas = db.query(FichaTecnica).filter(FichaTecnica.cliente_id==c.id).all()
                                    for f in fichas:
                                        itens = db.query(ItemFichaTecnica).filter(ItemFichaTecnica.ficha_tecnica_id==f.id).all()
                                        for item in itens:
                                            db.delete(item)
                                        db.delete(f)
                                    db.delete(c)
                        ...
                        with col3:
                            if FICHAS_ANINHADAS_DISPONIVEL:
                                ...
                                if usada > 0:
                                    ...
                                    if st.session_state.get('admin_mode', False):
                                        if st.button("🔥 FORÇAR", key=f"fdel_f{f.id}"):
                                            itens_que_usam = db.query(ItemFichaTecnica).filter(ItemFichaTecnica.ficha_ingrediente_id==f.id).all()
                                            for item in itens_que_usam:
                                                db.delete(item)
                                            itens = db.query(ItemFichaTecnica).filter(ItemFichaTecnica.ficha_tecnica_id==f.id).all()
                                            for item in itens:
                                                db.delete(item)
...
st.markdown("---")
versao = "v2.3 🔓 Admin" if FICHAS_ANINHADAS_DISPONIVEL else "v2.3"
st.markdown(f"<div style='text-align:center;color:#888;'><p>Ficha Técnica PRO {versao}</p></div>", unsafe_allow_html=True)l_ficha, gerar_pdf_ficha
from gerenciador_logos import salvar_logo_cliente, carregar_logo_cliente
